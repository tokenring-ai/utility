...
export default class KeyedRegistryWithMultipleSelection<
	T = any,
> extends KeyedRegistry<T> {
	...
	enableItems = (names: Iterable<string>): void => {
		for (const name of names) {
			if (name.endsWith("*")) {
				...
				const matchingItems = Object.keys(this.items).filter((itemName) =>
					itemName.startsWith(prefix),
				);
				if (matchingItems.length === 0) {
					throw new Error(
						`Couldn't enable ${name}: no items found matching prefix`,
					);
				}
	...
	disableItems = (names: Iterable<string>): void => {
		for (const name of names) {
			if (name.endsWith("*")) {
				...
				if (matchingItems.length === 0) {
					throw new Error(
						`Couldn't disable ${name}: no items found matching prefix`,
					);
				}
				for (const matchingItem of matchingItems) {
	...
	setEnabledItems = (names: Iterable<string>): void => {
		...
		for (const name of names) {
			if (name.endsWith("*")) {
				...
				if (matchingItems.length === 0) {
					throw new Error(
						`Couldn't set enabled items with ${name}: no items found matching prefix`,
					);
				}
